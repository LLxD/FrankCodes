...
def verifica_distancia():
    distancias = {}
    while True:
        if (sensor3.distance_centimeters() < 40):
            motorA.stop()
            motorB.stop()
           ''' for i in range(1:5):
                motorB.run_timed(speed_sp=200, time_sp=i*100)
                motorA.run_timed(speed_sp=-200, time_sp=i*100)
                distancia = sensor3.distance_centimeters()
                distancias[i] = distancia
            for i in range(1:5):
                motorB.run_timed(speed_sp=-200, time_sp=i*100)
                motorA.run_timed(speed_sp=200, time_sp=i*100)
            x = min(distancias, key=distancias.get)
            motorB.run_timed(speed_sp=200, time_sp=i*100)
            motorA.run_timed(speed_sp=-200, time_sp=i*100)
            motorA.stop()
            motorB.stop()
            while (sensor4.distance_centimeters() > 6):
                motorA.run_forever(speed_sp=200)
                motorB.run_forever(speed_sp=200)
            teste_garra()
            break'''
